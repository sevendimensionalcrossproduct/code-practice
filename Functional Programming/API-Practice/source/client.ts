...
declare global {
  interface Window {
    ...
    createUser: typeof promptAndCreateUser;
    getUsername: typeof promptAndGetUsername;
    updateUsername: typeof promptAndUpdateUsername;
    deleteUsername: typeof promptAndDeleteUsername;
    wipe: typeof wipeUsers;
    hideUsers: typeof hideUsers;
  }
}
...
export const promptAndCreateUser = async() => {
  crud.specifyUserData('Enter new username', 'Invalid username', async(newUsername : string) => {
      const newUser : crud.userJson = { userId: -1, userName: newUsername }; //ID calculated by API
...
export const promptAndGetUsername = async() => {
  crud.specifyUserData('Enter ID to get:', 'No ID provided', async (userId : string) => {
    try{
      await crud.findId(userId, (user : crud.userJson) => alert(`Username for user ID ${userId}: ${user.userName}`));
    } catch (error) {
      console.error('bad: ', error);
    }
  })
  ...
  crud.specifyUserData('Enter ID to update:','No ID provided' , async (userId:string) =>{
    try {
      await crud.findId(userId, async() => {
        ...
        crud.specifyUserData('Enter updated username', 'Invalid username', async(updatedUser: string) => {
...
export const promptAndDeleteUsername = async() => {
  crud.specifyUserData('Enter ID to delete:', 'No ID provided', async(userId : string) => {
    try{
      await crud.findId(userId, async (user : crud.userJson) => {
